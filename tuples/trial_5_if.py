...
listofgrades=[] #empty list of grades
highestscore=""

for i in listoftuples:
    listofgrades=listofgrades+[i[1],] #concatenating items in position 0 from listoftuples to list list of grades
    
#print(listofgrades) ['D', 'B', 'C', 'A', 'B', 'E']
a=0

for j in listofgrades:

    if listofgrades[j]>'A':
        print(listofgrades[j])

#if v<w and v<x and v<y and v<z:
#    print(v)

    #if listofgrades[j]=='A':
     #   highestscore=highestscore+listogrades[j]
...
#indexofA=listofgrades.index('A') #position 3

#subjectA=listoftuples[indexofA] #('French', 'A')

#print("The subject with the highest mark is",subjectA[0]) #French
